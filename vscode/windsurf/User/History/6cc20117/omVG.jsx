...
import CodingInterviewBox, {
  formatOpenAIResponse,
  formatUserMessage,
} from "../components/CodingInterviewBox";
...
const CodingProblemPage = ({ problemId }) => {
  ...
  const addUserMessageToChatHistory = (message) => {
    setChatHistory((prev) => [...prev, formatUserMessage(message)]);
  };

  useEffect(() => {
    const socket = io("http://localhost:3001");
    setChatSocket(socket);
    installSocketHandlers(socket, addInterviewerResponseToChatHistory);
    return () => {
      socket.disconnect();
    };
  }, []);
...
const installSocketHandlers = (socket, addInterviewerResponseToChatHistory) => {
  socket.on("connect", () => {
  ...
  socket.on("candidate", (message) => {
    ...
    addInterviewerResponseToChatHistory(message);
  });

  socket.on("getProblemId", (data, ackCB) => {
    ackCB(problemId);
  });
};
