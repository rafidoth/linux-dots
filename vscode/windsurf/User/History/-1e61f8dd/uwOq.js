...
export async function fetchExamsByQuizset(quizsetId, userId) {
  try {
    ...
    if (data.success) {
      const validatedData = examsByQuizsetSchema.safeParse(
        data.data.map((exam) => ({
          ...exam,
      ...
      if (validatedData.success) {
        ...
        throw new Error(
          `[ZOD VALIDATION] , ${JSON.stringify(validatedData.error?.issues)}`
        );
      }
    } else {
      return [];
...
export async function getExamStatus(examId, userId) {
  try {
    ...
    if (data.success) {
      ...
      throw new Error(
        `[ZOD VALIDATION] , ${JSON.stringify(data.error?.issues)}`
      );
    }
  } catch (error) {
