...
export async function someoneVisitedExamUrl(examId, userId) {
  try {
    const headers = {
      "Content-Type": "application/json",
      "x-user-id": userId,
    ...
    const response = await postRequest(
      `/api/exam/someoneVisitedExamUrl`,
      "POST",
      JSON.stringify({ examUrl }),
      headers
    );

    if (!response.ok) {
      throw new Error("Failed to update exam visit count");
    }

    return await response.json();
  } catch (error) {
    console.error("Failed to update exam visit count:", error);
    throw error;
  }
}
