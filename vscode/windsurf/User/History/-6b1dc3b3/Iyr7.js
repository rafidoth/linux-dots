        ...
        if (
          ...
          if (Array.isArray(lastApproach.tasks)) {
            const counts = lastApproach.tasks.reduce(
              ...
              { question: 0, coding: 0, timecomplexity: 0, spacecomplexity: 0 }
            );
          
            if (counts.question < 2) {
              console.log(
                "[Approach check] This approach doesn’t contain the required 2 question tasks"
              );
            }
            if (counts.coding < 1) {
              console.log(
                "[Approach check] This approach doesn’t contain the required coding task"
              );
            }
            if (counts.timecomplexity < 1) {
              console.log(
                "[Approach check] This approach doesn’t contain a time-complexity task"
              );
            }
            if (counts.spacecomplexity < 1) {
              console.log(
                "[Approach check] This approach doesn’t contain a space-complexity task"
              );
            }
          
            // Optional helper flag if you still want a single boolean
            ...
            const prompt = `  
              ...
              if [candidate is asking a question]
                ...
                In this approach ${lastApproach.approachType}, 
                candidate 
