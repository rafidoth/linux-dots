...
const QuizCard = ({ quiz, index, contextFieldOpen }) => {
  ...
  return (
    <div
      ...
      } break-inside-avoid p-4 shadow-md inset-shadow-2xs border border-gray-500/30    mb-4 rounded-3xl`}
    >
      ...
      <div className="text-xl mb-4 font-semibold">{quiz.question}</div>
      ...
      <div className="flex items-center gap-2 mt-3 justify-evenly">
        <MyBtn
          className={`bg-muted-foreground text-white rounded-sm m w-42 hover:bg-muted-foreground/90`}
      ...
      <div className="flex items-center gap-2 mt-3 justify-evenly">
        <MyBtn
          className={`bg-gray-800 text-white rounded-sm m w-42 hover:bg-muted-foreground/90`}
          onClick={() => setShowAnswer(!showAnswer)}
        >
