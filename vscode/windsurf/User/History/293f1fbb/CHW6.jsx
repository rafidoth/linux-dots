...
export default function ExamCreationDialog({ quizsetId, quizsetTitle }) {
  ...
  return (
    <Dialog>
      ...
      <DialogContent
        className={`font-inter font-[500] text-lg min-w-fit px-10  border-none outline-none bg-neutral-300 rounded-2xl`}
      >
        <DialogHeader>
          ...
          {existingExams.length > 0 && (
            <div className="flex flex-col gap-y-2 items-start">
              ...
              <div className="flex flex-col gap-y-2">
                  ...
                  (exam, i) =>
                    i < 2 && (
                      <Button
                        key={exam.id}
                        variant="outline"
                        className="w-full h-fit text-left flex justify-between items-center  hover:bg-accent cursor-pointer"
                ...
                {existingExams.length > 2 && (
                  <Button
                    variant="outline"
                    className="w-full h-fit text-left flex justify-center items-center  hover:bg-accent cursor-pointer"
                    onClick={() => navigate(`/xm/${quizsetId}`)}
                  >
                    See All Exams
                  </Button>
                )}
