...
func checkIfPositionIsValid(position Position, grid [][]byte) bool {
	if position.rowNo < 0 || position.colNo < 0 || position.rowNo >= len(grid) || position.colNo >= len(grid[0]) {
		return false
	}
	return true
}
...
func DFS(grid [][]byte,visited [][]bool, position Position){
	if checkIfPositionIsValid(position, grid) {
