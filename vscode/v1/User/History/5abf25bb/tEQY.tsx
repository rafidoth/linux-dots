    ...
    <div className="p-6">
      <div>
        ...
        {todaysQuizsets.map((quizset) => {
          const d = convertDate(quizset.created_at);
          return (
            <div key={quizset.id} className="flex flex-col w-[300px]">
              <h1 className="font-semibold text-xl">{quizset.title}</h1>
              <div className="flex justify-end w-full">
                <span className="bg-amber-500 border-jigao backdrop-blur-xl px-1 rounded-md">
                  {d.date} {d.month}, {d.year}
                </span>

                <span className="bg-jigao/40 border-jigao backdrop-blur-xl px-1 rounded-md">
                  {d.time}
                </span>
              </div>
            </div>
