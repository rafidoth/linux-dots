    ...
    CREATE TABLE Mentor_Availability (
        ...
        CONSTRAINT uq_mentor_availability UNIQUE (mentor_id, available_date, start_time)
    );
    ...
    CREATE TABLE One_on_One_Session_Platform (
        meeting_id CHAR(36) DEFAULT (UUID()) NOT NULL,
        one_on_one_session_id CHAR(36) NULL,
        platform VARCHAR(255) NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        CONSTRAINT pk_meetings PRIMARY KEY (meeting_id),
        CONSTRAINT fk_meetings_one_on_one FOREIGN KEY (one_on_one_session_id) REFERENCES One_On_One_Sessions(one_on_one_session_id) ON DELETE CASCADE,
        CONSTRAINT uq_meetings_one_on_one UNIQUE (one_on_one_session_id)
    );
